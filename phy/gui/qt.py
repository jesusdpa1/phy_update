"""Qt utilities with Qt5/Qt6 compatibility."""
...
# Use compatibility layer for Qt constants
Qt = QtCompat
...
def create_app():
    ...
    if QT_APP is None:  # pragma: no cover
        # Setup Qt environment compatibility fixes before creating QApplication
        setup_qt_environment()
...
def run_app():  # pragma: no cover
    ...
    global QT_APP
    return QT_APP.exit(Qt.exec_app(QT_APP))
...
def _wait_signal(signal, timeout=None):
    ...
    # Use compatibility layer for exec method
    Qt.exec_event_loop(loop)
...
def _block(until_true, timeout=None):
    ...
    while not until_true() and (default_timer() - t0 < timeout):
        ...
        app.processEvents(Qt.AllEvents, int(timeout * 1000))
    if not until_true():
...
def _load_font(name, size=8):
    """Load a TTF font using the compatibility layer."""
    if name in _FONTS:
        return _FONTS[name]

    font = load_font_compatible(name, size)
    if font:
        _FONTS[name] = font
...
def show_box(box):  # pragma: no cover
    """Display a dialog using compatibility layer."""
    return _button_name_from_enum(Qt.exec_dialog(box))
...
def screenshot(widget, path=None, dir=None):
    ...
    # Use compatibility layer for screenshot
    if screenshot_widget_compatible(widget, path):
        logger.info(f'Saved screenshot to {path}')
        return path
    else:
        logger.warning(f'Could not take screenshot of {type(widget).__name__}')
        return None
...
def _get_icon(icon, size=64, color='black'):
    ...
    if not op.exists(output_path):  # pragma: no cover
        ...
        # Use compatibility function for text size
        width, height = get_text_size_compatible(draw, hex_icon, font)
        ...
        if org_size != size:
            # Use compatibility function for image resampling
            resampling = get_image_resampling()
            out_image = out_image.resize((org_size, org_size), resampling)
...
class WebPage(CompatWebPage):
    """A Qt web page widget with compatibility layer."""

    pass


class WebView(QWebEngineView):
    """A generic HTML widget with Qt5/Qt6 compatibility."""
...
def screenshot(widget, path=None, dir=None):
    ...
    try:
        if is_qt6():
            ...
            try:
                # For QWindow and other window types, use QScreen.grabWindow()
                ...
                try:
                    # Try using render() method for custom widgets
                    ...
                    # Last resort: screenshot the entire screen area where the widget is
                    screen = QGuiApplication.primaryScreen()
        ...
        logger.warning(f'Could not take screenshot of {type(widget).__name__}: {e}')
        return None
...
def _get_icon(icon, size=64, color='black'):
    ...
    if not op.exists(output_path):  # pragma: no cover
        ...
        # Handle PIL API changes
        try:
            # Newer PIL versions
            bbox = draw.textbbox((0, 0), hex_icon, font=font)
        ...
        # If necessary, scale the image to the target size
        if org_size != size:
            # Handle PIL API changes for resize
            try:
                # Newer PIL versions
                out_image = out_image.resize(
                    (org_size, org_size), Image.Resampling.LANCZOS
                )
            except AttributeError:
                # Older PIL versions
